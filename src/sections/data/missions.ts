...
const sideMission = (name: string, offset?: number) =>
  new Mission(name, [], { offset });
...
const homelandDefense3 = sideMission('Homeland Defense - 3', 2)
...
const touristOpenRoad = sideMission('Tourist: Open Road', 2)
  .tourist()
  .objective('In {nh}, extract 1 time.')
...
const broadKnowledge2 = sideMission('Broad Knowledge - 2', -2)
...
const broadKnowledge3 = sideMission('Broad Knowledge - 3', -2)
...
const broadKnowledge4 = sideMission('Broad Knowledge - 4', -2)
...
const swiftStorm = sideMission('Swift Storm', -2)
  .inASingleMatch()
...
const probeTheShadows = sideMission('Probe the Shadows', 2)
  .inASingleMatch()
...
const unravelingTheWeb = sideMission('Unraveling the Web', 2)
  .objective('In {zd}, investigate _8 different GTI/Delta Force/Dagger emblems_')
  .follows(probeTheShadows);

const exposingTheShadowsZeroDam = sideMission('Exposing the Shadows: Zero Dam', 2)
  .objective('In {zd}, extract with any _2 Secret Protocol Crates_ in total.')
  .follows(unravelingTheWeb);

const touristBridgingTheDivide = sideMission('Tourist: Bridging the Divide', 2)
  .tourist()
  .withoutDying()
